...
from .message_type import ChatCompletionResponse, ModelInfo, ModelsResponse, SearchRequest, SearchResponse, SearchResult
...
async def call_backend(model_name: str, endpoint_type: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=timeout_duration) as client:
            logger.debug(f"Calling real backend: {backend_url} with model {payload['model']}")
...
async def create_chat_completion(request: Request):
    ...
    try:
        ...
        if not isinstance(messages, list):
            logger.error(f"Messages must be an array, got: {type(messages)}")
            raise HTTPException(status_code=400, detail=f"Messages must be an array, got: {type(messages)}")

        for msg in messages:
            if not isinstance(msg, dict):
                logger.error(f"Invalid message format (not a dict): {type(msg)}")
                raise HTTPException(status_code=400, detail="Invalid message format: message must be an object")
            ...
            if not role:
                ...
                raise HTTPException(status_code=400, detail="Message missing required field: 'role'")

            if not content:
                logger.error(f"Message missing 'content' field: {msg}")
                raise HTTPException(status_code=400, detail="Message missing required field: 'content'")
        ...
        raise HTTPException(status_code=400, detail=f"Error parsing request: {str(e)}") from e
...
async def create_search(request: Request):
    ...
    if request_obj.model not in AVAILABLE_MODELS or AVAILABLE_MODELS[request_obj.model]["type"] != "search":
        raise HTTPException(status_code=400, detail=f"Model {request_obj.model} not found or not a search model")
